// @ts-ignore
import alternativePageScript from "./scripts/alternativepage.inline"
import styles from "./styles/alternativepage.scss"
import { QuartzComponent, QuartzComponentConstructor, QuartzComponentProps } from "./types"
import { classNames } from "../util/lang"

const AlternativePage: QuartzComponent = ({ displayClass, fileData }: QuartzComponentProps) => {
  const alternativePage = fileData.frontmatter?.alternativePage as string | undefined

  const handleClick = () => {
    if (alternativePage) {
      window.location.href = alternativePage
    }
  }
  ...
  return (
    <button
      onClick={handleClick}
      class={classNames(displayClass, "alternativepage")}
    >
      <svg
        fill="#000000"
        version="1.1"
...
AlternativePage.beforeDOMLoaded = alternativePageScript
AlternativePage.css = styles

export default (() => AlternativePage) satisfies QuartzComponentConstructor
